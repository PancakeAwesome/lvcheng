 ...
 * CaptchaRender.
 */
public class CaptchaRewriteRender extends Render {

	private static String captchaName = "lvvc";
	...
	public static void setCaptchaName(String captchaName) {
		...
		CaptchaRewriteRender.captchaName = captchaName;
	}
	...
	public void render() {
		...
		Redis.use().setex(captchaName, 600, vCode);
		cookie.setMaxAge(-1);
		cookie.setPath("/");
		try {
	...
	public static boolean validate(Controller controller, String userInputCaptcha) {
		...
		System.out.println("controller.getCookie(captchaName)===="+controller.getCookie(captchaName));
		boolean result = userInputCaptcha.equals(controller.getCookie(captchaName));
		//boolean result = userInputCaptcha.equals(Redis.use().get(captchaName));
		if (result == true) {
